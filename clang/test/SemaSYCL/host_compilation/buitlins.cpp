// RUN: %clangxx -fsycl-device-only  -D __SYCL_HOST_COMPILATION__ -mllvm -sycl-host-compilation -Xclang -fsycl-int-header=%t.h -sycl-std=2020 -mllvm -sycl-opt -S -emit-llvm  -o - %s | FileCheck %s
