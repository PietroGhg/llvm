// RUN: %clangxx -fsycl-device-only -fsycl-host-compilation  -Xclang -fsycl-hc-header=%t-hc.h -Xclang -fsycl-int-header=%t.h -Xclang -fsycl-int-footer=%t-footer.h %s -o %t.bc
