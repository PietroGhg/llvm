// RUN: %clangxx -fsycl-device-only  -D__SYCL_HOST_COMPILATION__ -fsycl-host-compilation  -mllvm -hc-header=%t-hc.h -mllvm -sycl-host-compilation -Xclang -fsycl-int-header=%t.h -Xclang -fsycl-int-footer=%t-footer.h %s -o %t.bc
