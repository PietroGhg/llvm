// RUN: %clangxx -fsycl-device-only  -D__SYCL_HOST_COMPILATION__ -fsycl-targets=x86_64-unknown-unknown  -mllvm -hc-header=%t-hc.h -mllvm -sycl-host-compilation -Xclang -fsycl-int-header=%t.h -Xclang -fsycl-int-footer=%t-footer.h %s -o %t.bc
// RUN: %clangxx -include %t.h -include %t-hc.h -I %sycl_include -I %sycl_include/sycl -I %opencl_include_dir %s -O2 -c -o %t-host.o
