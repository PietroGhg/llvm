// RUN: %clangxx -fsycl-device-only -fsycl-native-cpu  -Xclang -fsycl-native-cpu-header=%t-hc.h -Xclang -fsycl-int-header=%t.h -Xclang -fsycl-int-footer=%t-footer.h %s -o %t.bc
